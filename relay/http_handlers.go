...
func (h *HTTP) handleStandard(w http.ResponseWriter, r *http.Request, start time.Time) {
	...
	h.mtx.Lock()
	bodyBuf := getBuf()
	// log.Printf("init body Buf  : %d", bodyBuf)
	_, _ = bodyBuf.ReadFrom(r.Body)

	// log.Printf("body Buf  : %d", bodyBuf)
	...
	// log.Printf("body buf %d", bodyBuf)
	outBuf := getBuf()
	// log.Printf("Out buf init write bytes %d", outBuf)
	...
	// log.Printf("Out buf after write bytes %d", outBuf)
	...
	for _, b := range h.backends {
		...
		// log.Printf("List points  : %s", points)
		// h.mtx.Lock()
